...
def getProfileURLs(companyName):
    ...
    invisibleEmployees = source.find_all('div', class_='artdeco-card pv5 pl5 pr1 mt4')
    for invisibleguy in invisibleEmployees:
        title = invisibleguy.findNext('div', class_='org-people-profile-card__profile-info').contents[0].strip('\n').strip('  ')
...
if __name__ == "__main__":
    companies = ['tesla-motors'] #, 'microsoft', 'amazon', 'tesla-motors', 'google', 'nvidia', 'berkshire-hathaway', 'meta', 'unitedhealth-group'
    login()
    employees = {}
